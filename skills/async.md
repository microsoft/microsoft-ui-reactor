...
name: reactor-async
description: >
  Async data patterns in Reactor — UseResource (single fetch), UseMutation
  (writes + optimistic updates), UseInfiniteResource (cursor pagination),
  and Pending (bubble-up fallback). Load this when the agent needs to fetch,
  cache, paginate, or mutate server data.
---
