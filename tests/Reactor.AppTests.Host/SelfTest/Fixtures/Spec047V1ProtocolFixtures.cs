...
///
/// <para>V1 is the unconditional production path. The harness creates
/// a fresh <c>ReactorHost</c> (and thus a fresh <c>Reconciler</c>) per
/// fixture so each scenario runs with isolated reconciler state.</para>
///
/// <para>Each fixture asserts the element renders to the expected DP
/// values and callbacks fire on the expected interactions. Diff
/// failures indicate a parity bug that must be fixed before Phase 1
/// closes.</para>
/// </summary>
