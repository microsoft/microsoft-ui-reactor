...
<kbd>Tab</kbd> moves through the row's editable cells and its Save/Cancel buttons
— read-only columns are skipped — without committing anything.
...
either edit mode it does exactly what <kbd>Tab</kbd> does — commit the cell in
`EditMode.Cell`, commit nothing in `EditMode.Row` — only walking to the previous
editable cell instead of the next one.
