...
> The former `Grid(["120", "1*", "Auto"], ...)` string-form overload was
> removed — use the typed helpers (`GridSize.Px(120)` / `GridSize.Star(1)` /
> `GridSize.Auto`), which catch typos at compile time.
...
| `.Margin(n)` / `.Margin(thickness)` | Outer spacing (all sides) |
| `.Padding(n)` / `.Padding(thickness)` | Inner spacing (all sides) |
