...
The reconcile-highlight overlay is documented in
[Dev Tooling](dev-tooling.md); reach for it first when "something
...
**Reach for the overlay before a trace.** The
[reconcile-highlight](dev-tooling.md) overlay costs nothing to
enable and will solve "which component is re-rendering too much"
without you ever opening a profiler. A trace is the right tool when
you need elapsed microseconds; the overlay is the right tool when
you need "which one".
...
- **[Dev Tooling](dev-tooling.md)** — The reconcile-highlight overlay plus the rest of the `mur` CLI.
- **[DevTools Internals](devtools-internals.md)** — How the overlay reads the same provider events the profilers do.
