...
The window icon is the Win32 `HICON` Windows shows in the taskbar, the Alt-Tab
switcher, and Task Manager. Set it declaratively with `icon:` on `ReactorApp.Run`,
or with `WindowSpec.Icon` for a secondary window:
...
- A packaged app does **not** get its window icon from `Package.appxmanifest`.
  The manifest's `Square44x44Logo` drives the *taskbar button* through package
  identity, which bypasses the window handle entirely — so without an explicit
  icon the Alt-Tab entry still shows a generic glyph.
