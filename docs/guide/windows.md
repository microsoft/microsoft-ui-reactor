...
This trips people up, so it is worth being precise. The window icon and the
taskbar button are **not** always the same picture, and which one wins depends on
...
| Task Manager | window icon | manifest logo, via package identity |

So for a **packaged** app, setting `icon:` alone is not enough to change the
taskbar button — the shell resolves that through package identity and never looks
at the window handle. Ship a `Square44x44Logo` that matches your `.ico` as well,
or the two surfaces will legitimately disagree and it will look like the window
icon "did not apply". For an **unpackaged** app there is no package identity, so
the window icon drives every surface and `icon:` is all you need.
...
- A packaged app does **not** get its window icon from `Package.appxmanifest`.
  ...
  button looks right.
- `<ApplicationIcon>` alone sets the icon Explorer shows for the `.exe`. Reactor's
