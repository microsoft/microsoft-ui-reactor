...
Every WinUI event attribute has a matching Reactor fluent. The pattern is
straightforward — drop the leading `On` from the property name:

| XAML | Reactor |
|------|---------|
| `<Button Click="OnClick"/>` | `Button("…").Click(handler)` |
| `<TextBox TextChanged="OnTextChanged"/>` | `TextField(text, setText).TextChanged(handler)` |
| `<ListView SelectionChanged="OnSelectionChanged"/>` | `ListView<T>(...).SelectionChanged(handler)` |
| `<ComboBox SelectedIndexChanged="OnSelectedIndexChanged"/>` | `ComboBox(...).SelectedIndexChanged(handler)` |
| `<CheckBox IsCheckedChanged="OnIsCheckedChanged"/>` | `CheckBox(value, setValue).IsCheckedChanged(handler)` |
