...
> The former `Func(ctx => ...)` factory was removed in this release. Replace it
> with `Memo(ctx => ...)` for the common case or `RenderEachTime(ctx => ...)`
> when you specifically want the always-re-render shape.
...
- **[Hooks](hooks.md)** — Next: deep dive into [UseState](reference/hooks/UseState.md), [UseReducer](reference/hooks/UseReducer.md), [UseEffect](reference/hooks/UseEffect.md), and all the hooks
