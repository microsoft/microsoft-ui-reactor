...
Build and install the VSIX using the developer instructions in the [VS extension README](https://github.com/microsoft/microsoft-ui-reactor/blob/main/src/vs-reactor/README.md). Phase 1 may ship unsigned for dev distribution, so Visual Studio may show an unsigned-extension warning. For local validation, install into the Experimental hive first (`/RootSuffix Exp`, which F5 on `Reactor.VsExtension` does for you), then open a Reactor sample or app project and show the **Reactor Preview** tool window.
...
Packaged projects work with no project changes: `dotnet new reactor` generates a packaged
app that carries the Windows App SDK run support, and the preview supplies the rest. A
project converted to MSIX by hand needs that run support
(`Microsoft.Windows.SDK.BuildTools.WinApp`) added before the preview can attach.
...
Planned follow-ups include a Tools → Options page, richer solution-load behavior, optional marketplace distribution, expanded ARM64/signing validation, surfacing the reactive reference-graph overlay (today exposed by the VS Code preview and the `references` devtools tool) inside the embedded tool window, and enough soak time to decide whether the VSIX should graduate out of "rough experiment" status.
