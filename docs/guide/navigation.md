...
| `Frame(...)` | element | Raw WinUI Frame, for interop with pages that already have a `.xaml` file; `.Navigating`, `.Navigated`, `.NavigationFailed`. Not for navigation inside a Reactor app — see below. |
...
### The target must have a `.xaml` file

`Frame` is WinUI's navigation control, and WinUI resolves the target type through
the XAML type metadata the XAML compiler generates. A `Page` declared only in C#
never reaches that metadata, so WinUI cannot resolve it — and calling
`Frame.Navigate` on an unresolvable type terminates the process with an access
violation rather than throwing, which no `try`/`catch` and no
`Application.UnhandledException` handler can intercept.

Reactor checks first and refuses, reporting through `.NavigationFailed(...)`:
...
**This is a deliberate boundary, not a gap.** Reactor's navigation system is
`UseNavigation<TRoute>` + `NavigationHost` — it needs no XAML, no `Page`
subclass, and no parameterless constructor, and it gives you a typed back stack,
lifecycle hooks, transitions and deep linking that `Frame` cannot. `Frame` exists
for apps that already have XAML pages and want to host them inside a Reactor
tree. See [spec 011](https://github.com/microsoft/microsoft-ui-reactor/blob/main/docs/specs/011-navigation-design.md)
§"Why WinUI Frame is not the answer" for the three further `Frame` constraints
(the `IPage` hard-cast, parameterless-constructor activation, and the absence of
extension points) that make it unsuitable as Reactor's navigation model.

Wiring `.NavigationFailed(...)` also marks the failure handled, so a refused
navigation is reported to your handler instead of tearing the render pass down.
Leave it unwired and the failure surfaces as an ordinary managed exception you
can break on.
...
expand/collapse through `.ItemExpanding(handler)` /
`.ItemCollapsed(handler)`, and the adaptive layout reports its
`Minimal`/`Compact`/`Expanded` switches through
`.DisplayModeChanged(handler)`:
...
`ItemInvoked`, `ItemExpanding` and `ItemCollapsed` receive the item's tag;
`DisplayModeChanged` receives the new `NavigationViewDisplayMode`. All four
clear when passed `null`.
...
If you drive the whole view with `.WithNavigation(nav, routeToTag, tagToRoute)`,
settings routing is opt-in through its fourth argument:
`.WithNavigation(nav, routeToTag, tagToRoute, () => Route.Settings)`. It is a
separate argument rather than a `SettingsTag` lookup through `tagToRoute`
because that delegate has to return a route for every string it is handed —
most implementations throw on an unrecognized tag — so it has no way to decline.

The rest of the control's chrome is declarative too:
`.BackButtonVisible(visible)` and `.PaneToggleButtonVisible(visible)` — what you
reach for when a `TitleBar` already owns that chrome — plus
`.PaneVisible(visible)`, `.AlwaysShowHeader()`, `.TitleBarAutoPadding()`,
`.SelectionFollowsFocus()`, `.OverflowLabelMode(mode)`,
`.ShoulderNavigation(mode)`, the `PaneHeader` and `ContentOverlay` slots,
and the `.CompactPaneLength(px)` / `.OpenPaneLength(px)` /
`.CompactModeThresholdWidth(px)` / `.ExpandedModeThresholdWidth(px)` metrics.

> The fluents drop the `Is` prefix that the underlying properties carry
> (`.BackButtonVisible(false)` sets `IsBackButtonVisible`), matching the
> `TitleBar` fluents of the same name. Both elements control the same chrome,
> so the call reads identically whichever one owns it.
