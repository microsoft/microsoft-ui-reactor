...
| `Win2DCanvas(...)` / `Win2DAnimatedCanvas(...)` | Optional [Win2D canvas](win2d-canvas.md) surfaces from `Microsoft.UI.Reactor.Advanced` (import `using static Microsoft.UI.Reactor.Advanced.Factories;`). |
...
| API | Purpose |
|---|---|
| `ReactorApp.Run<TComp>(title, width?, height?, fullScreen?, icon?, configure?)` | Single-window app. Omit `width`/`height` to take the OS default size. |
