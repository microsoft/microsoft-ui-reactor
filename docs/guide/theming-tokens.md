...
the [styling](styling.md) layer can hand a brush to a control without
caring which theme is active. The most common mistake is reaching for
a literal because "I'll just hardcode this one"; the
[REACTOR_THEME_001](rules-of-reactor.md) analyzer catches that at
...
[WinUI Fluent colors](https://learn.microsoft.com/en-us/windows/apps/design/style/colors).
...
![Token swatches — light theme](images/theming-tokens/swatch-grid-light.png)
![Token swatches — dark theme](images/theming-tokens/swatch-grid-dark.png)
...
> **Caveat:** A hardcoded color string in `.Background("#0066CC")` skips the
> theme swap entirely. The button stays blue when the user flips to
> dark mode, and white text on a now-still-blue background still
> satisfies WCAG against light's neutral — but the dark-mode neutral
> behind it makes the page look broken. Use `Theme.Accent` (or the
> matching `Theme.Ref(key)`) on `.Background` / `.Foreground` /
> `.WithBorder`; [REACTOR_THEME_001](rules-of-reactor.md) flags the
> literal at build time so the regression never reaches review.

## How resolution works

[`ThemeRef`](styling.md) is a readonly record struct holding the
resource key. When a modifier like `.Background(token)` reaches the
reconciler, the framework walks from the materialized
[`FrameworkElement`](components.md) up the visual tree looking for the
...
[`ActualThemeChanged`](animation.md); Reactor subscribes and updates
the brush on every element that referenced a `ThemeRef`. Components
that branch on the scheme use [`UseColorScheme`](hooks.md):
...
[`RequestedTheme`](styling.md) — useful for a video player surface or
...
The literal locks the value to one theme; the token participates in
the swap. The analyzer surfaces this at build time so a regression
can't ship; treat the warning as an error in CI.
...
- **[Styling](styling.md)** — Previous in the styling chain: the
  modifier surface that consumes `ThemeRef` values.
- **[Animation](animation.md)** — Next: animating between themed
  values; how the brush transitions through a theme swap.
- **[Accessibility](accessibility.md)** — Contrast budgets that
  determine which token to reach for in each surface.
- **[Devtools Internals](devtools-internals.md)** — How the dev-menu's
  theme-swap toggle drives the same resolver this page documents.
- **[Rules of Reactor](rules-of-reactor.md)** — Where the
  REACTOR_THEME_001 analyzer is listed alongside the rest of the
  enforced idioms.
