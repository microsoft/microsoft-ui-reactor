...
value in `Optional<T>.Of(value)` internally. Most app code therefore keeps
compiling unchanged.
...
> `T` → `Optional<T>` conversion when the intent is unambiguous.
...
|---|---|---|
| `SelectedIndex = -1` | `SelectedIndex = Optional<int>.Unset` | `SelectedIndex = Optional<int>.Of(-1)` |
| `SelectedPageIndex = 0` as "not set" | `SelectedPageIndex = Optional<int>.Unset` | `SelectedPageIndex = Optional<int>.Of(0)` |
| `IsChecked = false` as "not set" | `IsChecked = Optional<bool>.Unset` | `IsChecked = Optional<bool>.Of(false)` |
| `IsChecked = null` on tri-state checkbox | `IsChecked = Optional<bool?>.Unset` | `IsChecked = Optional<bool?>.Of(null)` |
| `Value = 0.0` as "not set" | `Value = Optional<double>.Unset` | `Value = Optional<double>.Of(0.0)` |
| `Value = ""` / `Text = ""` as "not set" | `Value = Optional<string>.Unset` / `Text = Optional<string>.Unset` | `Optional<string>.Of("")` |
| `Date = null` on `CalendarDatePicker` | `Date = Optional<DateTimeOffset?>.Unset` | `Date = Optional<DateTimeOffset?>.Of(null)` |

Reference-type null is explicit. For an `Optional<Brush>` property,
`with { Background = null }` means `Optional<Brush>.Of(null)`, not `Unset`.
...
**Force-assert sentinels explicitly.** If `-1`, `false`, `null`, or `""` is the value you want Reactor to assert, wrap it with `Optional<T>.Of(...)` so future readers do not mistake it for "not set".
