...
| [Win2D canvas](win2d-canvas.md) (`Reactor.Advanced`) | Immediate-mode drawing, game loops, and virtual tiled surfaces that retained WinUI controls cannot represent cheaply. | Ordinary retained UI — stay with elements, modifiers, and animation. |
...
removes the modifier clears the local value, handing the property back
to whatever the style (or inheritance) supplies. `.Set` is
