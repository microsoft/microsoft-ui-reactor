...
Forms are the Microsoft.UI.Reactor (Reactor) surface most apps spend most of their UI time inside.
...
```csharp
// Don't — there is no `initial:` parameter, and an uncontrolled TextBox
// has no way to be read, reset, or pre-filled by its parent:
...
- **[Hooks](hooks.md)** — [UseState](reference/hooks/UseState.md), [UseMemo](reference/hooks/UseMemo.md), and other hooks that power form logic
