...
per-callback. Prefer `Translation` for position-as-state — manipulation events
can drain several ticks between re-renders, so a `setOffset(offset + g.Delta)`
pattern reads a stale `offset` closure and produces visibly choppy panning.
For accumulation across separate gestures, hold the committed position in a
ref and snapshot it on `onEnded`:
