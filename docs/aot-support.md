...
- **The runtime canary.** CI's `AOT Selftests` job (`.github/workflows/ci.yml`) publishes `tests/Reactor.AppTests.Host` with `PublishAotInternal=true` and runs the full selftest suite against the NativeAOT binary on every PR. `SelfTestRunner.DefaultAotSkipPatterns` mutes the known reflection-bound fixtures (PropertyGrid auto-discovery, the two reflection-based Devtools tools — `fire` and `state` — Issue142 XAML metadata, plus the two framework cases under investigation); any *new* failure fails the job. If you intentionally need to add a skip, document the bucket in the comment above `DefaultAotSkipPatterns` and re-probe with `tests/Reactor.AppTests.Host/probe-aot-skips.ps1`. Treat that list as a liability, not a parking lot: a stale entry is AOT coverage silently switched off, so re-probe periodically and delete whatever now passes.
