...
| **Devtools `properties` / `setProperty` DP discovery** | The tools locate `DependencyProperty` statics by reflection. WinUI's are CsWinRT-projected static *properties*, and ILCompiler emits no reflection metadata for them unless something roots `PublicProperties` on those types, so under AOT the lookups find nothing: `properties` returns `{"count":0,"properties":[]}` and by-name reads answer "No DependencyProperty '…' found". Works under JIT. The rest of the property-tool surface — `resources`, `setResource`, `styles`, `ancestors`, and value formatting/parsing — is AOT-safe. Affected fixture: `Devtools_PropertyToolsDpDiscovery`. | Issue #1109 |
