...
- **Theming** — `ThemeRef` tokens, style caching. Brushes resolved through `XamlControlsResources` work under AOT; Reactor ships the WindowsAppSDK#6394 publish workaround in its own targets, so no app-side setup is needed (see [Required publish-time workarounds](#required-publish-time-workarounds) below).
- **Built-in WinUI controls** — `NavigationView`, `TabView`, `Pivot`, `TreeView`, `Expander`, `InfoBar`, `MenuBar`, `BreadcrumbBar`, `RefreshContainer`, `TeachingTip`, `ColorPicker`, `NumberBox`, `RatingControl`, `SplitButton`, etc. all mount and update under AOT, because Reactor's shipped targets mitigate WindowsAppSDK#6394 automatically. `{TemplateBinding}` against built-in DPs works; private DPs declared in a separate third-party assembly without an `IXamlMetadataProvider` are still broken (Issue #142).
...
| **Theme resource lookup (`Theme.X`, `ThemeRef.Resolve`)** | Works — Reactor's shipped `_ReactorCopyWinUIResourcesToPublish` target puts the project `.pri` in the publish output (see [Required publish-time workarounds](#required-publish-time-workarounds)). Reactor's library itself is AOT-clean here. | WindowsAppSDK#6394 |
...
The devtools `properties` / `setProperty` tools find `DependencyProperty` statics by
reflecting over static members, and trimming removes the metadata for members nothing
roots. On a NativeAOT build the lookups therefore come back empty even for DPs that
plainly exist (issue #1109). Under JIT — where the devtools inner loop lives, since an
AOT publish takes 10-15 minutes — discovery works.

**The tool says so rather than looking broken.** When the inspected app is a NativeAOT
build, an empty listing carries a `notice`, and a failed by-name lookup appends the same
explanation to its error. Both are conditioned on the app actually being NativeAOT
(`RuntimeFeature.IsDynamicCodeSupported`), so a JIT build never sees them, and an AOT
build that does root its DP statics stays silent too. Without this the caller gets the
same `not found` they would for a typo and goes hunting for a mistake they did not make.

**If you need DP inspection in an AOT build**, root the DependencyProperty statics with an
ILLink descriptor. That was measured on `samples/apps/hello-world-aot-devtools-on`
(win-x64, `TrimMode=full`, `OptimizationPreference=Size`) before being left out of the
#1109 fix, so the cost is known: rooting only the DP getters adds **+1.5 MB** (11.655 →
13.196 MB) and covers all 1851 DPs across 311 WinUI types, while the blunt
`preserve="all"` over `Microsoft.WinUI` costs +16.7 MB for the same capability. Gating
the descriptor on the `Reactor.DevtoolsSupport` switch keeps devtools-off apps
byte-identical. It is not shipped because a generated descriptor pinned to a Windows App
SDK version needs regenerating on every SDK bump — ongoing friction for everyone, to
serve a scenario the tool is not aimed at.
...
When publishing an unpackaged WinUI 3 app (`WindowsPackageType=None`), the WindowsAppSDK build pipeline generates `$(AssemblyName).pri` into the build output but does *not* copy it into the publish directory. The MakePRI step is conditioned on `AppxPackage=true`, so unpackaged apps fall through. Same applies to `.xbf` files generated by the XAML compiler.

**This is not AOT-specific.** It was originally found via `PublishAot=true`, but a plain `dotnet publish -c Release` loses the `.pri` just the same — measured on the stock `reactorapp` template, where the published app dies at startup with `0xC000027B` (`STATUS_APPLICATION_INTERNAL_EXCEPTION`) inside native XAML.
...
Upstream tracking: [microsoft/WindowsAppSDK#6394](https://github.com/microsoft/WindowsAppSDK/issues/6394) — closed 2026-09-24 as fixed in `Microsoft.Windows.SDK.BuildTools.MSIX` **1.7.260903100**, then **REOPENED 2026-09-25**: *"Reopening because fix on MRT core version used when not enabling msix tooling is still not released."* That is upstream confirming the gate described below — the fix exists, but only on the MSIX-tooling path.
...
Opt out — for example if you drive the copy yourself — with:
...
That gate is not an oversight, and the upstream "fixed" is not wrong: **the official WinUI templates set `<EnableMsixTooling>true</EnableMsixTooling>` by default**, so the mainstream WinUI app is genuinely fixed by upgrading. Reactor's template ships `WindowsPackageType=None` for xcopy deployment and does not set that flag, so the platform's targets do not run for it.

**Turning the flag on is not yet sufficient either, which is the reason this target still exists.** The two halves of the upstream fix shipped separately: `AddProjectPriToResolvedFileToPublish` is in 1.7.251221100, but `AddXbfFilesToResolvedFileToPublish` — the half that closed the issue — is only in **1.7.260903100**, and no `Microsoft.WindowsAppSDK` version pulls that yet. Measured: the newest SDK (2.5.1) resolves `Microsoft.Windows.SDK.BuildTools.MSIX/1.7.251221100`, and an unpackaged consumer with `EnableMsixTooling=true` on that version publishes its app `.pri` but **not** its own compiled `.xbf`. So doing it entirely "the upstream way" today still leaves half the gap open.

Measured on 1.7.260903100 (explicitly pinned) with a `-v:detailed` publish, counting how many times each fix target actually executes:
...
**So upgrading the build tools does not by itself remove the need for this workaround** — for an app in Reactor's default shape.
...
**When to delete the shipped target.** Not "when #6394 closes" — it already closed once, on 2026-09-24, and was reopened the next day precisely because the non-MSIX-tooling path is still unfixed. Delete this when **both** hold: a `Microsoft.WindowsAppSDK` release actually pulls build tools carrying the fix (as of 2026-09-25 the newest `Microsoft.WindowsAppSDK.Base`, 2.0.4, still pins `1.7.251221100`), **and** either those targets move outside the `MsixPackageSupport` gate or Reactor's template adopts `EnableMsixTooling=true`. Adopting the flag alone is not enough today — measured above, it publishes the `.pri` but not the `.xbf`. The `ConsumerPublishIncludesTheAppPriAndXbfSidecars` test in `tests/Reactor.IntegrationTests/Packaging/PriPackagingTests.cs` is the tripwire: its opt-out control asserts the app `.pri` and the consumer's own `.xbf` are *absent* when Reactor's target is disabled, so it starts failing the moment the platform begins doing this itself.
