...
| `bootstrap.ps1` | Derives the Windows App Runtime winget id from `WindowsAppSDKVersion` in `Directory.Build.props`. For 2.x that is the major-only id `Microsoft.WindowsAppRuntime.2`, which tracks in-place servicing (2.0 → 2.1 → 2.3). The major.minor id `Microsoft.WindowsAppRuntime.2.0` is a *different* winget package pinned to the 2.0 servicing line (still 2.0.1) and cannot satisfy an app built against 2.1.3, so it must not be used. Pinned by `tests/Reactor.Tests/WinAppSDKReferenceGuardTests.cs`. |
...
| **Standalone scaffolding pins** | The `dotnet new` template (`tools/Templates/.../Company.ReactorApp1.csproj`), the CLI's local-scaffold csproj string (`Reactor.Cli/Program.cs`), and the `Microsoft.WindowsAppSDK` install snippets in `docs/guide/index.md`, `SKILL.md`, and `samples/WinFormsInterop/README.md` each **directly** pin the SDK and do **not** inherit the central `WindowsAppSDKVersion`. A stale `2.0.1` / `2.0.*` pin is an **NU1605 downgrade** against the framework's new `>= 2.1.3` dependency — the `Integration Tests` (template smoke) and `bootstrap.ps1` CI jobs scaffold + restore and fail on it. These must be bumped in lockstep (done: `2.1.3` / `2.1.*`). Future SDK bumps should grep `Microsoft.WindowsAppSDK" Version=` and update every literal pin. <br><br>⚠️ **Corrected by [spec 063 §3.0](063-unpackaged-app-data.md#30-correction-to-spec-059-3).** *(The `2.1.3` / `2.1.*` values above are this spec's own 059-era state, kept as the historical record. The live floor is `WindowsAppSDKVersion` in `Directory.Build.props` — 2.2.0 as of spec 063 — and is now enforced by a guard test rather than by following this row.)* Two errors in this row: (1) the `dotnet new` template does **not** literally pin the SDK — it sets `WindowsAppSDKSelfContained=true` and references only `Microsoft.UI.Reactor`; (2) the prescribed `Microsoft.WindowsAppSDK" Version=` grep is **structurally blind to a second pin shape** — the `#:package Microsoft.WindowsAppSDK@<version>` file-based-app header — so this spec's own bump missed all **22 occurrences across 20 files**, including the **18 recipe files** packed into the **shipped** NuGet agent kit. They sat a full major-minor below the floor this spec raised, for an entire release cycle. Spec 063 fixes them and replaces the prose instruction with a guard test, because an enumeration written as prose is only ever as good as the last person's regex. |
...
| `ApplicationData.GetForUnpackaged()` (2.2.0) | Could replace bespoke file-path persistence for dock-layout / window-placement in unpackaged apps with a first-class `ApplicationData`. Worth a focused look, but orthogonal to this spec and gated on a 2.2.0 bump. <br><br>✅ **Adopted by [spec 063](063-unpackaged-app-data.md)** (2026-09-16), which bumps the SDK to 2.2.0 and adds `UnpackagedAppDataStore`. Note it adopts the `LocalPath` surface, *not* `LocalSettings` — the latter writes to a roaming registry hive on 2.2.0 (measured; WindowsAppSDK#6559). The fix lands at metapackage **2.3.1** / `Foundation` 2.3.5, **not** 2.5.1 as that issue's release notes claim; see [spec 063 §3.2](063-unpackaged-app-data.md) for the two-arm measurement and for why the boundary attaches to a `Foundation` version rather than a metapackage one. |
