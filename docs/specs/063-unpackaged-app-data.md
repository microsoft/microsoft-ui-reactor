...
| **Cross-process write safety** | **Fixed in this spec.** `JsonFileStore` did read-merge-write under a per-*instance* lock through a shared `.tmp`, so two processes writing different persistence ids could drop each other's entries — measured at 1 of 8 surviving under 8 concurrent writers. Its class doc also claimed a `FileShare.None` protection that was not in the code. The store now takes a named `CrossProcessWriteGuard` (a `Local\`-scoped mutex keyed on a hash of the store path) around the merge, stages through a per-process/thread temp file, and commits with `File.Replace` rather than `File.Move(overwrite: true)` — measured on Windows, `Move` cannot replace a destination another process holds open even when that reader grants delete sharing, whereas `Replace` can, so reads additionally open with `FileShare.Delete`. Both halves are required; either alone lets a concurrent reader silently kill a write. Acquisition is bounded at 5 s and degrades to writing unguarded rather than hanging the UI thread on close. **This fixes the default store**, so every existing unpackaged app benefits, not just the new one — which matters because this store's publisher/product identity is deliberately *shared*, making the collision far more reachable than under the process-name-keyed default. Mutation-verified: removing the guard returns the suite to 1-of-8 survivors. |
