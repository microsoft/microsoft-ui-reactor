...
| (not listed) | **The `#:package Microsoft.WindowsAppSDK@<version>` file-based-app header is a second pin shape entirely.** It does not match the `Microsoft.WindowsAppSDK" Version=` grep that spec 059 §3 prescribes, so PR #723 missed **every instance of it** — not just one. **22 literal occurrences across 20 files** were left at `2.0.1`, already a downgrade against 2.1.3, for a full release cycle: `SKILL.md`'s single-file-app snippet, 8 recipes under `skills/recipes/`, 10 under `plugins/reactor/skills/reactor-recipes/references/`, and 3 occurrences inside the single file `samples/apps/demo-script-tool/App/Resources/SystemPrompt.txt`. The **18 recipe files** (8 + 10) are **packed into the shipped NuGet agent kit** (`src/Reactor/Reactor.csproj`), so this was a user-facing `NU1605` downgrade for anyone who ran a recipe, not an internal-only staleness; the `SystemPrompt.txt` occurrences are not packed, but they teach an LLM which pin to emit into generated apps. All now `2.2.0`. |
...
| **Cross-process write safety** | **Fixed in this spec.** `JsonFileStore` did read-merge-write under a per-*instance* lock through a shared `.tmp`, so two processes writing different persistence ids could drop each other's entries — measured at 1 of 8 surviving under 8 concurrent writers. Its class doc also claimed a `FileShare.None` protection that was not in the code. The store now takes a `CrossProcessWriteGuard` around the merge: a **lock file** beside the store opened `FileShare.None`, not a named mutex — a mutex name carries a scope, and `Local\` would let the same user in two logon sessions take different objects for one file while `Global\` needs a privilege a standard-user process may not hold, whereas a lock file inherits the scope of the thing it protects and a killed holder is released by the OS. The lock file is **retained on disk**; only the handle is released. `FileOptions.DeleteOnClose` is deliberately *not* used: a delete-pending file makes Windows fail other opens with `ERROR_ACCESS_DENIED` rather than a sharing violation, which is indistinguishable from a real ACL failure and so cannot be retried safely — measured, it cost 2 of 8 concurrent writes to spurious give-ups. Writes stage through a per-process/thread temp file and commit with `File.Replace` rather than `File.Move(overwrite: true)` — measured on Windows, `Move` cannot replace a destination another process holds open even when that reader grants delete sharing, whereas `Replace` can, so reads additionally open with `FileShare.Delete`. Both halves are required; either alone lets a concurrent reader silently kill a write. Acquisition is bounded at 250 ms — short because `SavePlacement` runs on the UI thread at window close — and on timeout the write is **abandoned**, not forced: proceeding would merge a stale document and drop the peer's entry, which is the very defect being fixed, whereas skipping costs at most this window's placement, which best-effort already permits. **This fixes the default store**, so every existing unpackaged app benefits, not just the new one — which matters because this store's publisher/product identity is deliberately *shared*, making the collision far more reachable than under the process-name-keyed default. Mutation-verified: removing the guard returns the suite to 1-of-8 survivors. |
