...
- **Dispatch consolidation** — `ITemplatedItemsStrategy` and `IErasedTemplatedItemsStrategy` inherit from a new base `IItemsBinderStrategy`; `V1HandlerAdapter.DispatchChildrenMount` / `DispatchChildrenUpdate` and `DescriptorHandler.Mount` / `Update` collapse their per-strategy `is`-checks into one base-interface check. Future strategy markers (TreeChildren / PreMountedItems / TabItemsHost) plug into the same arm — M1 cost stays at one `is`-check + one interface call as new strategies arrive.
...
- **Port (8) `TreeView`** — needs a new `TreeChildren<TElement, TControl>` ChildrenStrategy (hierarchical, non-keyed). Builds `WinUI.TreeViewNode` recursively from `TreeViewNodeData`; selection / ItemInvoked / Expanding events wire inline. Strategy implements `IItemsBinderStrategy` so it plugs into the consolidated dispatch arm.
- **Port (9) `FlipView`** — needs a `PreMountedItems<>` ChildrenStrategy. FlipView pre-mounts all items into `FlipView.Items` directly (no CCC, no OC delta, no ItemsSource binding). Verify whether the existing `ItemsHost<>` covers it before designing a new strategy.
