...
{
    // Registered by a control family (today: Charting) to contribute element
    // checks the core shouldn't know about (issue #498). Null in apps that
    // never activate the subsystem, so the trimmer drops the concrete
    ...
    /// activates. See <see cref="IScanExtension"/>.
    /// </summary>
    internal static void RegisterScanExtension(IScanExtension extension)
        => s_scanExtension = extension;
    ...
    private static void Walk(Element? el, ScanContext ctx, List<A11yDiagnostic> findings)
    {
        ...
        // Chart-specific checks — delegated to the Charting subsystem's
        // registered scanner extension (issue #498). Null in chart-free apps,
        // so the core never statically references any Charting type.
        s_scanExtension?.Check(el, ctx, findings);
    ...
    private sealed class ScanContext : IScanContext
    {
        ...
        //    a registered subsystem extension without leaking the static class. ──
        string? IScanContext.GetAutomationId(Element el) => AccessibilityScanner.GetAutomationId(el);
