...
public sealed partial class Reconciler : IDisposable
{
        ...
        Microsoft.UI.Reactor.Elements.ResourceOverrides? newOverrides)
    {
        ...
        //
        // Membership is tested directly against the new Literals/ThemeRefs dictionaries
        // (O(1) ContainsKey) instead of materializing AllKeys.ToHashSet(), and removals
        // are deferred into a lazily-allocated list so the common no-removal case doesn't
        // allocate managed.ToArray() (perf #26, preserved in the loop below).
        if (managed.Count > 0)
        {
