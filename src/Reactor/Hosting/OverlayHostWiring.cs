...
/// <summary>
/// Single wrapper surface shared by every dev overlay. The host installs this
/// ONE wrapper when any overlay flag is on; each feature registers a
/// sub-renderer that paints into the shared <see cref="ContainerVisual"/>
/// attached to the wrapper's Canvas.
/// </summary>
/// <remarks>
...
internal sealed class OverlayHostWiring : IDisposable
{
    ...
    /// <summary>
    /// Reconcile the visible overlay sub-renderers with the current flag
    /// state. Called every render so when a feature flag goes off
    /// mid-session, its visuals are torn down promptly. The wrapper Canvas
    /// stays alive so long as any other overlay is still on.
    /// </summary>
