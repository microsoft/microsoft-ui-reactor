...
{
    internal sealed class StopPreviewCommand
    {
        public static async Task InitializeAsync(AsyncPackage package, CancellationToken ct)
        {
            ...
            {
                await OutputChannel.WriteLineAsync("IMenuCommandService unavailable; Stop Preview command not registered.").ConfigureAwait(true);
                return;
            }

            menuCommandService.AddCommand(new MenuCommand(Execute, new CommandID(PackageGuids.CommandSetGuid, CommandIds.StopPreview)));
        }
