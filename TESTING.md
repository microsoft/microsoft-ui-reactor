...
**4. Expected pass count.** As of 2026-08-21, an AOT run of the suite produces: 1447 fixtures total → 18 skipped, 1429 passed, 0 failed. Fifteen of those skips come from `DefaultAotSkipPatterns` (they are the ones tagged `# SKIP crashes/hangs under NativeAOT`); the remaining three are skipped by design on every run — the non-AOT run on the same commit reports the same three. The skip list covers fixtures that exercise subsystems documented as not-yet-AOT-clean in [`docs/aot-support.md`](docs/aot-support.md) (PropertyGrid auto-discovery, two Devtools fixtures whose reflection targets get trimmed — `fire`'s handler lookup and `state`'s hook read — `UseObservable` on POCOs, hot-reload state migration, XAML-metadata-dependent control hosting). When you fix one of those subsystems, drop the corresponding entries from `DefaultAotSkipPatterns` — `SelfTestRunner.ValidateDefaultSkipPatterns` aborts the run (TAP `Bail out!`, exit 1) if an entry stops matching a real fixture, but nothing detects an entry that has merely started *passing*, so re-probe periodically.
