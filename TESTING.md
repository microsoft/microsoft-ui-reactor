...
**4. Expected pass count.** As of 2026-08-20, an AOT run of the suite produces: 1445 fixtures total → 17 skipped, 1428 passed, 0 failed. Sixteen of those skips come from `DefaultAotSkipPatterns`; the seventeenth is `SelfTestVerdict_OnlySkips_PositiveControl`, which is skipped by design on every run. The skip list covers fixtures that exercise subsystems documented as not-yet-AOT-clean in [`docs/aot-support.md`](docs/aot-support.md) (PropertyGrid auto-discovery, two Devtools fixtures whose reflection targets get trimmed, `UseObservable` on POCOs, hot-reload state migration, XAML-metadata-dependent control hosting). When you fix one of those subsystems, drop the corresponding entries from `DefaultAotSkipPatterns` — `SelfTestRunner.ValidateDefaultSkipPatterns` aborts the run (TAP `Bail out!`, exit 1) if an entry stops matching a real fixture, but nothing detects an entry that has merely started *passing*, so re-probe periodically. The non-AOT run on the same commit skips only the positive control.
