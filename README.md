...
Reactor ships the public preview package `Microsoft.UI.Reactor` version `0.1.0-preview.11` on NuGet.org. The project template is still installed from source for now; `bootstrap.ps1` installs the `mur` CLI, packs/registers the local `reactorapp` template, and that template references the public preview package by default.
