...
| `REACTOR_DSL_004` | info | `.WithKey(item)` or `.WithKey(item.Key)` inside a `ForEach` whose items implement `IReactorKeyed` — the factory already assigns that exact key | Delete the call (there is a "Remove redundant .WithKey(...)" fix). Only these two spellings are reported; `.WithKey(item.Id)` or any other expression is treated as a deliberate override and left alone. |
