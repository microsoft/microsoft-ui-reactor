...
| `REACTOR_DSL_001` | warning | `Select(...)` or `ForEach(...)` projecting into a layout container without `.WithKey(...)` | `items.Select(i => Row(i).WithKey(i.Id)).ToArray<Element?>()`. Keys keep focus + animation state across reorders. Silent for `ForEach` over `IReactorKeyed` items — the factory keys those itself. |
...
| `REACTOR_DSL_004` | info | `.WithKey(item)` or `.WithKey(item.Key)` inside a `ForEach` whose items implement `IReactorKeyed` — the factory already assigns that exact key | Usually just delete the call. No auto-fix: if the receiver picks up a key inside a called factory, the call is an override and removing it would change the key. Only these two spellings are reported; `.WithKey(item.Id)` or any other expression is left alone. |
...
| `REACTOR_ICON_001` | warning | A `WindowIcon` source kind handed to a surface that cannot consume it: `FromResource` on a tray icon, taskbar overlay or thumbnail-toolbar button, or `FromBytes`/`FromRgba` on `WindowSpec.Icon` / `ReactorApp.Run(icon:)` / a jump-list entry | Each surface needs a different primitive — the three shell surfaces need a raw `HICON` (no `ms-appx:` URI), the window caption needs a filesystem path, and a jump-list logo needs a `Uri` or path. The mismatch compiles and is then silently dropped with only a `Debug.WriteLine`. Use `FromPath` (works everywhere), or `FromBytes`/`FromRgba` for the shell surfaces and `FromResource` for a packaged window icon. Only fires when the kind is provably known at the use site — a conditional, field, parameter or method result stays silent, as does either non-binary kind on a jump list (packaged vs unpackaged is a runtime property). |
...
| `REACTOR_MOD_002` | info | `.Set(c => c.Prop = v)` where a first-class `.Prop(v)` modifier exists — **or** an attached setter `.Set(c => Owner.SetProp(c, v))` whose receiver is a type `ElementPool` never holds, which reports `MOD_002` rather than `POOL_001` | Use the modifier. `.Set` setters re-run every render, are never unwound when a later render drops them, and `Element.SettersEqual` pins the element to the reconciler's update path. The fix rewrites the whole `.Set` — including multi-statement bodies — into a modifier chain. |
| `REACTOR_MOD_003` | warning | A common modifier that `ApplyModifiers` never writes to this element's control (`.Background(...)` on a `Rectangle`, `.Padding(...)` on a `Canvas` or a `FlexRow`, `.CornerRadius(...)` on an `Image`) | The call compiles and is silently discarded. On a shape use the paint modifier (`.Fill(...)` / `.Stroke(...)` / `.StrokeThickness(...)` — the fix does the rewrite); on a `Flex`/`FlexRow`/`FlexColumn` use `.FlexPadding(...)`; otherwise host the element in a control that supports it. Gates: `Background` → Panel/Control/Border; `Foreground` and fonts → Control/TextBlock; `Padding` → Control/Border/Grid/StackPanel/RelativePanel/TextBlock; `CornerRadius` → Control/Border/Grid/StackPanel/RelativePanel; `BorderBrush`/`BorderThickness` → Control/Border. |
| `REACTOR_POOL_001` | warning | `.Set` writes a property `ElementPool.CleanElement` clears on pool return — either an instance write `c.Prop = v` (layout/appearance: `Margin`, `Width`/`Height`, `Min`/`Max` sizes, `HorizontalAlignment`/`VerticalAlignment`, `Opacity`, `AccessKey`, `IsTabStop`, plus `IsEnabled` on `Control` receivers and `Padding`, `CornerRadius`, `BorderThickness`, `BorderBrush`, `Background` on their supported receivers) or an attached setter `Owner.SetProp(c, v)` (`AutomationProperties.*`, `ToolTipService.*`, `TitleBar.IsDragRegion`, `FlexPanel.*`). **Receiver-aware in two independent ways:** the `.Set` lambda parameter's *exact* type must be one `ElementPool` recycles — `TextBlock`, `RichTextBlock`, `StackPanel`, `Grid`, `Border`, `ScrollViewer`, `Canvas`, `Viewbox`, `ProgressBar`, `ProgressRing`, `Image`, `InfoBadge`, `Button`, `TextBox`, `ToggleSwitch` — *and* `CleanElement` must clear that particular property on that receiver, which is narrower for the receiver-gated rows above. Anything failing either test reports `REACTOR_MOD_002` instead — same modifier advice, minus the pool-return claim: the write lands and is never unwound, so what it costs is the element's structural skip (`Element.SettersEqual`), not the value. | Use the surviving modifier — the value is stored on `Element.Modifiers` and re-applied every render. Most map 1:1 and ship a fix (`.Padding(...)`, `.Background(...)`, `AutomationProperties.SetName` → `.AutomationName(...)`, `ToolTipService.SetToolTip` → `.ToolTip(...)`); the rest are diagnostic-only because the modifier's shape differs — `.PositionInSet(position, size)`, `.Required()`, `.Flex(grow: ...)`. |
...
|---|---|---|
| `ReactorSourceMap` | `true` in Debug, unset in Release | Runs the source-map interceptor generator, which stamps each DSL call site's file and line onto `Element.CallSite`. Read it back with `ReactorSourceMap.GetSource(uiElement)`. Follows the build configuration by default, mirroring WPF's `XamlDebuggingInformation`, but the generator is gated on this property alone — setting it `true` in Release does generate interceptors and embeds mapped source paths in the binary, so only do that for a build you do not distribute. Locations populate only while the runtime flag `ReactorSourceMap.Enabled` is on: the devtools verb sets it automatically, and a host with its own inspector can assign it directly or start the process with `REACTOR_SOURCEMAP=1`. A Debug build that never turns it on does no stamping and no extra cloning, so source mapping adds no allocation of its own; the one always-on cost is that `ElementExtras` carries a 24-byte `CallSite` field whenever an element allocates that bucket at all, flag or no flag (a leaf with no behavioral extras still allocates no bucket — see `ElementExtrasAllocationTests`). Costs roughly 0.5–0.6 ms per intercepted call site at build time — set it to `false` on a very large app if the edit loop matters more than element attribution. Setting it also implies the `InterceptorsNamespaces` entry the generator needs (see `CS9137` above). |
