...
// `ListView<T>(items, key, builder)` factory handles per-row keying for the
// reconciler — no manual `.WithKey` needed. `.ItemClick(handler)` is the
// fluent for ListView's primary item-tap event.
